...
router.post('/', async (req, res) => {
  ...
  if (Quantity < 0) return res.render('pages/HReq', { title: "Blood Bank", css1: "reg", css2: "", css3: "", scrp: "reg", msg: "Invalid Input!", isLogged: 1 });
          ...
          (err, rows) => {
            if (err || rows[0] == undefined) {
              ...
              db.all(insert_Query,
                (err) => {
                  if (err) {
                    return res.render("error", { title: "Blood Bank", css1: "reg", css2: "", css3: "", scrp: "reg", message: err.message });
                  }
                  else
